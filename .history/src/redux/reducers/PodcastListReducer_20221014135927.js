...
export const getPodcastList = () => (dispatch) => {
	...
	return axios
		.get('https://itunes.apple.com/us/rss/toppodcasts/limit=100/genre=1310/json'url)
		.then((response) => dispatch(getPodcastlistSuccess(response.data.feed.entry)))
		.catch((response) => dispatch(getPodcastListError(response.data.feed.entry)))
}
